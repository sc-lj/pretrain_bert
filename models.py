...
from transformers.modeling_bert import BertPreTrainingHeads, BertPreTrainedModel, BertPreTrainingHeads, BertLMPredictionHead,BertLayerNorm
...
class MTLRouting(nn.Module):
    ...
    def forward(self, batch, log=True):
        ...
        if log:
            self.log_summary_writer(batch_type, logs={'pretrain_loss': loss.item()})
        return loss
